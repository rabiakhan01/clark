...
const Card = () => {
    const data = cardData.map((items) => {
        return (
            <div className="flex flex-col text-primaryColor md:w-11/12 md:h-full xl:h-auto w-11/12 h-auto  bg-semiTransparent p-8 gap-y-4 rounded-sm">
    ...
    return (
        <div className="grid md:grid-rows-3 grid-rows-6  grid-flow-col gap-y-6 sm:gap-y-10 md:gap-x-0 gapy-x-2  place-items-center">
            {data}
        </div>
    );
}
